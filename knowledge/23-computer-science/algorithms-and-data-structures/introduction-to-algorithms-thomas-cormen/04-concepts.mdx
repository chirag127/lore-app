...
- If f(n) grows faster (and a f(n/b) <= c f(n)) → T(n) = Theta(f(n))
...
| Matrix-chain | O(n^2) | m[i,j] = min_{i<=k<j}(m[i,k] + m[k+1,j] + p_{i-1} p_k p_j) |
| LCS | O(mn) | c[i,j] = 0 (if i=0 or j=0); c[i-1,j-1]+1 (if x_i=y_j); max(c[i-1,j], c[i,j-1]) (else) |
| Optimal BST | O(n^3) | e[i,j] = min_{i<=r<=j}(e[i,r-1] + e[r+1,j] + w(i,j)) |
| Bitmask TSP | O(n^2 2^n) | dp[mask][v] = min(dp[mask\{v}][u] + w(u,v)) |
...
| Red-black tree height | 13 | h <= 2 lg(n+1) |
